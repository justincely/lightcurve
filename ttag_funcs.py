...
def ttag_image(in_data,xtype='XCORR',ytype='YCORR',pha=(2,30),bins=(1024,16384),times=None,ranges=((0,1023),(0,16384)),binning=(1,1),NUV=False):
...
def lightcurve( filename,xlim=(0,1024),ylim=(0,1024),step=5,extract=True,
    ...
    all_heights = [ hdu[1].header[ 'SP_HGT_%s'%(segment) ] for segment in ['A','B','C'] if 
                    key_exists( hdu[1].header, 'SP_HGT_%s'%(segment)  ) ]  
    all_locations = [ hdu[1].header[ 'SP_LOC_%s'%(segment) ] for segment in ['A','B','C'] if 
    ...
    for i in range(0,end,step)[:-1]:
        ...
        for loc,height in zip( all_locations, all_heights ):
            index = np.where( (hdu[1].data['TIME'] > i) & ( hdu[1].data['TIME'] < i+step) &
                              (hdu[1].data['XCORR'] > xlim[0]) & ( hdu[1].data['XCORR'] < xlim[1]) &
                              (hdu[1].data['YCORR'] > loc-(height/2) ) & ( hdu[1].data['YCORR'] < loc+(height/2) ) &
                              ( (hdu[1].data['WAVELENGTH'] > 1217) | (hdu[1].data['WAVELENGTH'] < 1214) ) )[0]
            sub_count += len(index)/float(step)
    ...
    if fluxcal:
        ...
        total_fluxcorr = np.sum( [ flux_hdu[1].data[i]['SENSITIVITY'] for i in index ] )
